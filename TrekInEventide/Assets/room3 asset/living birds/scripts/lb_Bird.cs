...
public class lb_Bird : MonoBehaviour {
	...
	public GameObject theDialogue1;
	...
	IEnumerator FlyToTarget(Vector3 target){
		...
		//birds fly up and away from their perch for 0.5 second before orienting to the next target
	...
	void OnGroundBehaviors(){
		...
		if(idle){
			...
			// Birds should fly to a new target about every 15 seconds
			if (Random.value < Time.deltaTime * 0.067)
			{
				StartCoroutine(DelayedFlyAway(15.0f));
			}
		}
	}

	IEnumerator DelayedFlyAway(float delayTime)
	{
